...
const EditCaseScreen = () => {
  ...
  useEffect(() => {
    const fetchCase = async () => {
      try {
        ...
        const { data } = await axios.get(`/api/cases/${caseId}`, config);
        setCaseDetails(data);
        ...
        setSupportType(data.supportType || 'money');
        setTargetAmount(data.targetAmount || '');
        setSituationImages(data.situationImages || []);
        ...
        if (data.neededItems && data.neededItems.length > 0) {
          setItems(data.neededItems);
        }
        
        setLoading(false);
      } catch (error) {
        setError(error.response?.data?.message || 'Không thể tải thông tin hoàn cảnh');
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    if (caseDetails && userInfo) {
      const isOwner = caseDetails.user?._id === userInfo._id || caseDetails.user === userInfo._id;
  ...
  const removeItem = (index) => {
    ...
    setItems(newItems.length ? newItems : [{ name: '', quantity: 1, unit: 'cái' }]);
  };
  ...
  const uploadFileHandler = async (e, type) => {
    ...
    try {
      ...
      setUploadError(error.response?.data?.message || 'Lỗi khi tải lên hình ảnh');
      setUploading(false);
    }
  };
  ...
  const submitHandler = async (e) => {
    ...
    if ((supportType === 'money' || supportType === 'both') && (!targetAmount || targetAmount <= 0)) {
      setError('Vui lòng nhập số tiền mục tiêu hợp lệ');
      return;
    }

    if ((supportType === 'items' || supportType === 'both') && (!items[0].name || items.some(item => !item.name || item.quantity <= 0))) {
    ...
    try {
      ...
      setError(error.response?.data?.message || 'Lỗi khi cập nhật hoàn cảnh');
      setLoading(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto">
      ...
      {success && (
        <Message variant="success" className="mb-4">
          Hoàn cảnh đã được cập nhật thành công! Đang chuyển hướng...
        </Message>
      )}
      ...
      <form onSubmit={submitHandler} className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          ...
          {(supportType === 'items' || supportType === 'both') && (
            <div className="mb-4">
              ...
              {items.map((item, index) => (
                <div key={index} className="flex flex-wrap items-center gap-2 mb-3 p-3 border border-gray-200 rounded-lg bg-gray-50">
                  <div className="flex-grow min-w-[200px]">
                    <label className="text-sm text-gray-600 mb-1 block">Tên vật phẩm</label>
                    <input
                      type="text"
                      value={item.name}
                      ...
                      required={supportType === 'items' || supportType === 'both'}
                    />
                  </div>
                  ...
                  <div className="w-24">
                    <label className="text-sm text-gray-600 mb-1 block">Đơn vị</label>
                    <select
                      value={item.unit}
        ...
        <div className="mb-6">
          <h2 className="text-lg font-semibold mb-4">Thông tin liên hệ và địa điểm</h2>
        ...
        <div className="mb-6">
          ...
          <div className="mb-4">
            <label className="block mb-1 font-medium">
              Hình ảnh minh họa tình huống <span className="text-red-500">*</span>
            </label>
            <input
              type="file"
        ...
        <div className="flex justify-end gap-3">
          ...
          >
            {loading ? <Loader color="white" size="small" /> : 'Lưu thay đổi'}
          </button>
        </div>
      </form>
    </div>
